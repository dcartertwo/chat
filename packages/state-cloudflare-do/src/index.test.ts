...
// ---------------------------------------------------------------------------
// Integration tests — require @cloudflare/vitest-pool-workers
// ---------------------------------------------------------------------------

describe.skip("integration tests (require Cloudflare Workers runtime)", () => {
  it("should create tables on first instantiation", () => {
    // Test with real ChatStateDO + miniflare
  });

  it("should handle concurrent requests via input gates", () => {
    // Verify single-threaded execution
  });

  it("should clean up expired entries via alarm", () => {
    // Test alarm handler
  });
});
